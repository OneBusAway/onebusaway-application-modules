...
public class NextBusApiBase {
	...
	protected List<AgencyAndId> processVehicleIds(String vehicleRef,
		...
		if (vehicleRef != null) {
			try {
				...
				AgencyAndId vehicleId = AgencyAndIdLibrary
						.convertFromString(vehicleRef);
	...
	protected <E> void processStopIds(String stopIdVal,
		...
		if (StringUtils.isNotBlank(stopIdVal)) {
			try {
				...
				AgencyAndId stopId = AgencyAndIdLibrary
						.convertFromString(stopIdVal);
				if (isValidStop(stopId)) {
					stopIds.add(stopId);
				} else {
					body.getErrors().add(
							new BodyError("No such stop: " + stopId.toString()
									+ ". "));
				}
			} catch (Exception e) {
			...
			if (stopIds.size() == 0)
				body.getErrors().add(
						new BodyError("You must provide a StopId."));
		}
	}
	...
	protected boolean isValidAgency(Body body, String agencyId) {
		...
		if (_transitDataService.getAgency(agencyId) == null) {
			body.getErrors().add(
					new BodyError("Agency parameter \"a=" + agencyId
							+ " is not valid."));
			return false;
		}
		return true;
	...
	protected String getServiceUrl() {
		...
		String serviceUrl = "http://" + host + ":" + port + "/api/v1/key/"
				+ apiKey + "/agency/1/command/";
		return serviceUrl;
	}
	...
	protected JsonObject getJsonObject(String uri) throws Exception {
		...
		JsonElement root = jp.parse(new InputStreamReader((InputStream) request
				.getContent())); // Convert the input stream to a
									// json element
