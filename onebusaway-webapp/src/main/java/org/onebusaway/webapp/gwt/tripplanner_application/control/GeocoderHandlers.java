...
public class GeocoderHandlers {
  ...
  private AsyncCallback<List<TripPlanBean>> _tripPlannerResultHandler;
  ...
  public void setTripPlannerResultHandler(
      AsyncCallback<List<TripPlanBean>> handler) {
  ...
  private void handleResults(boolean isDestination, JsArray<Placemark> locations) {
    ...
    System.out.println("isDestination=" + isDestination + " count="
        + locations.length());
  ...
  private void checkResults() {
    ...
    System.out.println("checking=" + _startLocation + " to "
    ...
    if (_startLocation != null && _destinationLocation != null) {
      ...
      service.getTripsBetween(from.getLatitude(), from.getLongitude(),
          to.getLatitude(), to.getLongitude(), constraints,
          _tripPlannerResultHandler);
    }
  }
