...
public class CacheableInterceptor {
  ...
  private Map<String, CacheEntry> _entries = new HashMap<String, CacheEntry>();

  private Map<Class<?>, CacheableObjectKeyFactory> _keyFactories;

  public void setCacheManager(CacheManager cacheManager) {
    _cacheManager = cacheManager;
  }

  public void setCacheKeyFactories(Map<Class<?>, CacheableObjectKeyFactory> keyFactories) {
      ...
      if (annotation != null) {
        ...
        try {
          ...
          throw new IllegalStateException("error instantiating CacheableObjectKeyFactory [type="
              + keyFactoryType.getName() + "] from CacheableKey [type=" + checkType.getName() + "]", ex);
        }
      }
  ...
  private T2<Method, Cacheable> getCacheableMethodAndAnnotation(ProceedingJoinPoint pjp) {
