...
public class ConstraintsParameterMapping {
  ...
  public static void addConstraintsToParams(OneBusAwayConstraintsBean constraints, Map<String, String> params) {

    long minDepartureTime = constraints.getMinDepartureTime();
    params.put(PARAM_TIME, Long.toString(minDepartureTime));

    params.put(PARAM_MAX_TRANSFERS, Integer.toString(constraints.getMaxTransfers()));
    params.put(PARAM_MAX_TRIP_LENGTH, Integer.toString(constraints.getMaxTripDuration()));
    params.put(PARAM_MAX_WALKING_DISTANCE, Double.toString(constraints.getMaxWalkingDistance()));

  }

  public static void addParamsToConstraints(Context context, OneBusAwayConstraintsBean constraints) {
