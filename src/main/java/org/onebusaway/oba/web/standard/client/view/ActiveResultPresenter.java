...
public class ActiveResultPresenter implements StateEventListener {
  ...
  private OneBusAwayStandardPresenter _control;
  ...
  public void setControlLayer(OneBusAwayStandardPresenter control) {
    _control = control;
  }
  ...
  private void fillEntryPanel(TimedLocalSearchResult result, FlowPanel panel) {
    ...
    if (entry.getRatingUrl() != null) {
      ...
      DivWidget rating = new DivWidget(entry.getRating() + "/" + entry.getMaxRating());
      ratingPanel.add(rating);
    }

    DivWidget time = new DivWidget("Travel time: ~" + (result.getTime() / 60) + " mins");
    ...
    directions.addClickListener(new DirectionsHandler(entry));
  ...
  private String getFormatedAddressForEntry(LocalSearchResult entry) {
    return entry.getAddress() + " " + entry.getCity() + ", " + entry.getRegion() + " " + entry.getZip();
  }
  ...
  private final class DirectionsHandler implements ClickListener {
