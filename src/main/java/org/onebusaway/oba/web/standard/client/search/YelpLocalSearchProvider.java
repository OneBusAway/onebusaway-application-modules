...
import org.onebusaway.common.web.common.client.rpc.JsonCallback;
import org.onebusaway.common.web.common.client.rpc.JsonLibrary;
...
public class YelpLocalSearchProvider implements LocalSearchProvider {
  ...
  public void search(LocationBounds bounds, String query, String category, LocalSearchCallback callback) {

    StringBuilder url = buildQuery(query, category);
    url.append("&tl_lat=").append(bounds.getLatMax());
    url.append("&tl_long=").append(bounds.getLonMin());
    url.append("&br_lat=").append(bounds.getLatMin());
    url.append("&br_long=").append(bounds.getLonMax());
    performQuery(url, callback);
  }

  public void search(LatLng center, String query, String category, LocalSearchCallback callback) {
    StringBuilder url = buildQuery(query, category);
  ...
  public void searchByRadius(LatLng center, double radius, String query, String category, LocalSearchCallback callback) {
  ...
  private void performQuery(StringBuilder url, LocalSearchCallback callback) {
    ...
    JsonLibrary.getJson("yelp" + (_requestId++), url.toString(), handler, _timeout);
  }
  ...
  private static class JsonHandler implements JsonCallback {
    ...
    private void checkStatusCode(JSONObject obj) throws YelpException, YelpStatusException {
    ...
    private LocalSearchResult parseResult(JSONValue value) throws YelpException {
      ...
      String ratingUrlSmall = JsonLibrary.getJsonString(obj, "rating_img_url_small");
