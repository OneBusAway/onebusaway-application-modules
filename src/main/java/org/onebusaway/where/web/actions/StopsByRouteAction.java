...
public class StopsByRouteAction extends ActionSupport {
  ...
  public String getNameAsString(NameBean name) {
    if (SelectionNameTypes.REGION_BETWEEN.equals(name.getType())) {
      return getNamesAsString(name);
    } else if (SelectionNameTypes.REGION_BEFORE.equals(name.getType())) {
      return "Before " + getNamesAsString(name);
    } else if (SelectionNameTypes.REGION_AFTER.equals(name.getType())) {
      return "After " + getNamesAsString(name);
    } else {
      return getNamesAsString(name);
    }
  }

  private String getNamesAsString(NameBean name) {
