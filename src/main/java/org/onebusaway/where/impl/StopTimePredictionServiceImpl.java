...
public class StopTimePredictionServiceImpl implements StopTimePredictionService {
  ...
  public void setOutputLog(Writer writer) {
    _outputLog = writer;
  }
  ...
  public void startup() {
    try {
      _receiver = new TimepointPredictionReceiver(_timepointPredictionServerName, _timepointPredictionServerPort);
      _receiver.start();
    } catch (IOException ex) {
      _log.log(Level.SEVERE, "error starting transit prediction data receiver", ex);
    }
  }
  ...
  public TimepointPredictionSummary getPredictionSummary(String tripId) {
    ...
    TimepointPredictionSummary summary = new TimepointPredictionSummary();

    if (element == null) {
      summary.setNumberOfPredictions(0);
      summary.setGoalDeviation(0);
      return summary;
    }
    ...
    for (TimepointPrediction prediction : predictions) {
      if (prediction.getGoalTime() != -1) {
        numberOfPredictions++;
        if (prediction.getScheduledTime() > scheduleTime) {
          scheduleTime = prediction.getScheduledTime();
          goalDeviation = prediction.getGoalDeviation();
        }
      }
    }

    summary.setNumberOfPredictions(numberOfPredictions);
    summary.setGoalDeviation(goalDeviation);

    return summary;
  }

  @SuppressWarnings("unchecked")
  public List<TimepointPrediction> getCachedTripEntry(String tripId) {
    Element element = _predictionsByTripCache.get(tripId);
    if (element != null)
      return (List<TimepointPrediction>) element.getValue();
    return null;
  }

  private Map<String, List<StopTimeInstance>> getStopTimeInstancesByTripId(List<StopTimeInstance> stopTimes) {
  ...
  @SuppressWarnings("unchecked")
  private Map<String, Map<String, List<Timepoint>>> getTimepointsByTripIds(Set<String> tripIds) {
    ...
    Map<String, Map<String, List<Timepoint>>> byTripId = getTimepointsByGtfsTripIdAndTimepointId(missingTimepoints);
  ...
  private Map<String, Map<String, List<Timepoint>>> getTimepointsByGtfsTripIdAndTimepointId(List<Timepoint> timepoints) {

    Map<String, Map<String, List<Timepoint>>> byTripId = new HashMap<String, Map<String, List<Timepoint>>>();

    for (Timepoint timepoint : timepoints) {

      String tripId = timepoint.getGtfsTripId();
  ...
  @SuppressWarnings("unchecked")
  private Map<String, List<TimepointPrediction>> getPredictionsByTripIds(Set<String> tripIds) {
  ...
  private void applyPredictions(String tripId, List<StopTimeInstance> stis,
      Map<String, List<Timepoint>> timepointsByTimepointId, List<TimepointPrediction> predictions) {
    ...
    for (TimepointPrediction prediction : predictions) {
      ...
      // If goalTime == -1, it means the prediction is not
      // available/applicable/usable.
      if (prediction.getGoalTime() != -1) {
  ...
  private double getInterpolatedGoalDeviation(SortedMap<Double, Integer> offsetBy, double distance) {
  ...
  private class TimepointPredictionReceiver extends SddReceiver {
    ...
    public TimepointPredictionReceiver(String serverName, int serverPort) throws IOException {
    ...
    public void extractedDataReceived(Hashtable ht, String serialNum) {
      ...
      if (ht.containsKey("PREDICTIONS")) {
        ...
        while (data.next()) {
          ...
          if (_outputLog != null) {
            try {
              _outputLog.write(data.getRowString() + "\n");
            } catch (IOException ex) {
              throw new IllegalStateException(ex);
            }
          }
  ...
  private static class TimepointPredictionComparator implements Comparator<TimepointPrediction> {
