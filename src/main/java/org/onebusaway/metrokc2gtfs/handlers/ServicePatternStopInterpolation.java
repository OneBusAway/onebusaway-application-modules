...
import org.onebusaway.metrokc2gtfs.model.MetroKCTransLink;
...
public class ServicePatternStopInterpolation {
  ...
  private ShapePointHandler _shapePointHandler;
  ...
  public ServicePatternStopInterpolation(TranslationContext context, ServicePatternKey pattern) {
    _context = context;
    _pattern = pattern;
    ...
    _stopHandler = _context.getHandler(StopHandler.class);
    _shapePointHandler = _context.getHandler(ShapePointHandler.class);
  }
  ...
  public SortedMap<Indexed<MetroKCStop>, StopTimepointInterpolation> run() {
    ...
    if (false) {
      for (Map.Entry<MetroKCPatternTimepoint, List<MetroKCTPIPath>> entry : _tpiPathsByPatterns.entrySet()) {
        ...
        System.out.println(pt.getTimepointId() + " " + pt.getSequence() + " " + pt.getTpiId());
        List<MetroKCTPIPath> tpips = entry.getValue();
        for (MetroKCTPIPath tpip : tpips)
          System.out.println("  " + tpip.getTransLink() + " " + tpip.getSequence() + " " + tpip.getFlowDirection());
      }
    }
    ...
    RouteSchedulePatternId rspid = new RouteSchedulePatternId(sp.getRoute(), sp.getSchedulePatternId());
    ...
    while (!_unassignedStops.isEmpty()) {
      ...
      for (Indexed<MetroKCStop> element : minElements) {
        ...
        if (false) {
          ...
          System.out.println(CSVLibrary.getAsCSV(score, _pattern.getId(), route.getShortName(),
  ...
  private void computeLinkInterpolations() {
    ...
    List<MetroKCPatternTimepoint> patterns = new ArrayList<MetroKCPatternTimepoint>(_tpiPathsByPatterns.keySet());
    ...
    for (int timepointIndex = 0; timepointIndex < patterns.size() - 1; timepointIndex++) {
      ...
      for (MetroKCTPIPath path : paths) {
        ...
        LinkInterpolation partial = new LinkInterpolation(link, nodeFrom, nodeTo, from.getTimepointId(),
            to.getTimepointId(), timepointIndex, path.getFlowDirection());
  ...
  private void snapStopsToAssignedOrNearestTransLink() {
    ...
    for (MetroKCOrderedPatternStop ops : _stops) {
      ...
      MetroKCStop stop = _stopHandler.getEntity(ops.getStop());
      ...
      if (pptFlag) {
        if (stopIndex == 0) {
          link = _firstLink;
        } else if (stopIndex == _stops.size() - 1 || stopIndex == _stops.size() - 2) {
          link = _lastLink;
        } else {
      ...
      if (data == null || data.size() == 0) {
        ...
        for (MetroKCTransLink validLink : _linksToInterpolation.keySet()) {
          ...
          double d = Math.sqrt(Math.pow(nodeA.getX() - nodeB.getX(), 2) + Math.pow(nodeA.getY() - nodeB.getY(), 2));
          m.add(d, validLink);
        }
   ...
   * consecutive stops
   */
  private List<List<Indexed<MetroKCStop>>> getClusters(Collection<Indexed<MetroKCStop>> elements) {
  ...
  private void rMostLikelyAssignment(Map<Indexed<MetroKCStop>, LinkInterpolation> assignments,
      Min<Map<Indexed<MetroKCStop>, LinkInterpolation>> m, List<Indexed<MetroKCStop>> elements, int index) {

    if (index >= elements.size()) {
      SortedMap<Indexed<MetroKCStop>, Double> d = new TreeMap<Indexed<MetroKCStop>, Double>(_distFromStart);
  ...
  private double getScore(SortedMap<Indexed<MetroKCStop>, Double> dFromStart, Indexed<MetroKCStop> ins, double len) {
  ...
  private void checkResults() {
    ...
    for (Map.Entry<Indexed<MetroKCStop>, StopTimepointInterpolation> entry : _results.entrySet()) {
      ...
      if (offset < prev) {
        ...
        Route route = _spHandler.getRouteByServicePatternKey(_pattern);
        String pre = "route(" + route.getShortName() + ")sp(" + _pattern.getId() + ")";

        System.out.println(pre + "stop(" + prevSti.getStop() + ")link(" + fromLink.getId() + ")");
        System.out.println(pre + "stop(" + sti.getStop() + ")link(" + toLink.getId() + ")");

        System.out.println("    prev=" + prevKey + " " + getDesc(prevSti) + " offset=" + prev + " first="
            + _firstPass.contains(prevKey));
        System.out.println("    next=" + key + " " + getDesc(sti) + " offset=" + offset + " first="
            + _firstPass.contains(key));
        System.out.println("    diff=" + (prev - offset));

        goCrazy(fromLink.getId(), toLink.getId(), _stopHandler.getEntity(prevSti.getStop()).getTransLink(),
            _stopHandler.getEntity(sti.getStop()).getTransLink());
      }
  ...
  private String getDesc(StopTimepointInterpolation sti) {
    String row = CSVLibrary.getAsCSV(sti.getTimepointFrom().getTimepoint(), sti.getTimepointTo().getTimepoint(),
        sti.getRatio());
    return row;
  }

  private void goCrazy(int... linkIds) {
    ...
    for (Map.Entry<MetroKCPatternTimepoint, List<MetroKCTPIPath>> entry : _tpiPathsByPatterns.entrySet()) {
      ...
      List<MetroKCTPIPath> tpips = entry.getValue();
      for (MetroKCTPIPath tpip : tpips) {
        String hit = linkSet.contains(tpip.getTransLink()) ? " HIT!" : "";
        System.out.println("  " + tpip.getTransLink() + " " + tpip.getSequence() + " " + tpip.getFlowDirection() + hit);
        List<MetroKCShapePoint> points = _shapePointHandler.getShapePointsByTransLinkId(tpip.getTransLink());
        if (tpip.getFlowDirection() == 0) {
          ...
          for (int i = points.size() - 1; i >= 0; i--) {
            MetroKCShapePoint point = points.get(i);
            System.out.println("    " + point.getLat() + " " + point.getLon());
          }
        }
      }
    ...
    for (int linkId : linkIds) {
      ...
      List<MetroKCShapePoint> points = _shapePointHandler.getShapePointsByTransLinkId(linkId);
      for (MetroKCShapePoint point : points) {
        System.out.println(point.getLat() + " " + point.getLon());
      }
    }
