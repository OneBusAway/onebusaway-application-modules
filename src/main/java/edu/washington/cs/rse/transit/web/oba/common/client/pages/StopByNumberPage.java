 ...
 * the License.
 */
package edu.washington.cs.rse.transit.web.oba.common.client.pages;
...
import edu.washington.cs.rse.transit.web.oba.common.client.widgets.DivPanel;
...
public class StopByNumberPage extends AbstractPageSource implements
  ...
  public void onSuccess(StopWithArrivalsBean bean) {
    ...
    List<PredictedArrivalBean> arrivals = bean.getPredictedArrivals();
    ...
    List<PredictedArrivalBean> active = new ArrayList<PredictedArrivalBean>();

    for (PredictedArrivalBean pab : arrivals) {
    ...
    for (PredictedArrivalBean pab : active) {
      ...
      String route = Integer.toString(pab.getRoute());
  ...
  private int getStopId(final Context context) throws PageException {
    if (!context.hasParam(ID_KEY))
      throw new PageException(_msgs.stopByNumberPageNoStopNumberSpecified());
    try {
      return Integer.parseInt(context.getParam(ID_KEY));
    } catch (NumberFormatException ex) {
      throw new PageException(
          _msgs.stopByNumberPageInvalidStopNumberSpecified());
    }
  }
  ...
  protected void handleInitialStopInformation(StopWithArrivalsBean swab,
    ...
    StopAreaBean bean = swab.getStop();
    ...
    if (!bean.getNearbyStops().isEmpty()) {
      _nearbyStops.add(new DivWidget(constants.stopByNumberPageNearbyStops()));
      for (StopBean s : bean.getNearbyStops()) {

        String target = getTarget("stop", ID_KEY, s.getId());
        String label = getCombinedIntersectionName(s) + " - "
            + s.getDirection() + " " + constants.stopByNumberPageBound();
  ...
  private String getArrivalLabel(PredictedArrivalBean pab, long now) {
  ...
  private String getArrivalStatusLabelStyle(PredictedArrivalBean pab, long now) {
  ...
  private class OrderClickHandler implements ClickListener {
    ...
    public void onClick(Widget arg0) {
      ...
      params.put(ID_KEY, Integer.toString(_stopId));
  ...
  private class RouteFilterClickHandler implements ClickListener {
    ...
    public RouteFilterClickHandler(int route) {
      _route = route;
    }
