...
import edu.washington.cs.rse.transit.common.offline.Graph;
...
public class StopSelectionServiceImpl implements StopSelectionService {
  ...
  public StopSelectionTree getStopsByRoute(int routeNumber)
    ...
    for (ServicePatternBlock block : blocks) {
      ...
      handleServicePatterns(byDest, block.getServicePatterns());
    }
  ...
  private void handleServicePatterns(StopSelectionTree tree,
      List<ServicePattern> servicePatterns) {

    Graph<StopLocation> graph = new Graph<StopLocation>();

    for (ServicePattern pattern : servicePatterns) {

      List<StopLocation> stops = _dao.getStopLocationsByServicePattern(pattern);

      StopLocation prevStop = null;
    ...
    List<StopLocation> ordered = graph.getTopologicalSort(null);

    Map<StopLocation, SortedMap<Layer, Region>> regions = _dao.getRegionsByStops(ordered);
  ...
  private void extendTree(List<StopLocation> stops,
  ...
  private void extendTreeByStreet(StopSelectionTree tree,
      List<StopLocation> stops, int from, int to) {

    for (int i = from; i < to; i++) {
      StopLocation stop = stops.get(i);
      StreetName main = stop.getMainStreetName();
      SelectionName mainName = new SelectionName(
          SelectionNameTypes.MAIN_STREET, main.getCombinedName());
      StopSelectionTree mainTree = tree.getSubTree(mainName);

      StreetName cross = stop.getCrossStreetName();
      SelectionName crossName = new SelectionName(
          SelectionNameTypes.CROSS_STREET, cross.getCombinedName());
      ...
      SelectionName name = new SelectionName(
          SelectionNameTypes.STOP_DESCRIPTION, id, desc);
