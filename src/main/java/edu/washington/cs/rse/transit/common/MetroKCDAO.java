...
public class MetroKCDAO {
   ...
   */
  @SuppressWarnings("unchecked")
  public Set<StopLocation> getStopLocationsByServicePatternBlock(
      ServicePatternBlockKey key, boolean includeTransLinks) {

    StringBuilder query = new StringBuilder();
    query.append("SELECT stop FROM");
    query.append("  ServicePatternBlock block,");
    query.append("  ServicePattern sp,");
    query.append("  OrderedPatternStops ops,");
    query.append("  StopLocation stop");
    if (includeTransLinks)
      query.append("  LEFT JOIN FETCH stop.transLink");
    query.append(" WHERE");
    query.append("  block.id = :id");
    query.append("  AND sp in elements(block.servicePatterns)");
    query.append("  AND ops.route = sp.route");
    query.append("  AND ops.schedulePatternId = sp.schedulePatternId");
    query.append("  AND ops.stop = stop");

    return new HashSet<StopLocation>(_t2.findByNamedParam(query.toString(),
        "id", key));
  }
  ...
  public Map<ServicePattern, Long> getServicePatternsByChangeDateAndRouteWithTripCounts(
      ChangeDate serviceRevision, Route route) {

    List<?> rows = _t2.find(
        "SELECT sp, count(*) FROM ServicePattern sp, Trip trip WHERE sp.id.changeDate = ? AND sp.route = ? AND trip.servicePattern = sp GROUP BY sp.id",
        new Object[] {serviceRevision, route});

    Map<ServicePattern, Long> counts = new HashMap<ServicePattern, Long>();

    for (Object row : rows) {
      Object[] values = (Object[]) row;
      ServicePattern p = (ServicePattern) values[0];
  ...
  @SuppressWarnings("unchecked")
  public List<ServicePatternBlock> getAllServicePatternBlocks() {
    return _t2.find("from ServicePatternBlock block");
  }

  @SuppressWarnings("unchecked")
  public List<ServicePatternBlock> getServicePatternBlocksByRoute(Route route) {
    return _t2.findByNamedParam(
        "from ServicePatternBlock block where block.id.route = :route",
        "route", route);
  }
  ...
  @SuppressWarnings("unchecked")
  public List<BlockTrip> getTripBlocksByServicePattern(
      ServicePattern servicePattern) {
    return _t2.findByNamedParam(
        "SELECT bt FROM BlockTrip bt WHERE bt.id.trip.servicePattern = :sp",
        "sp", servicePattern);
  }

  @SuppressWarnings("unchecked")
  public List<BlockTrip> getTripBlocksByIds(Set<Integer> ids) {
    return _t2.findByNamedParam(
        "SELECT bt FROM BlockTrip bt WHERE bt.id.id IN (:ids)", "ids", ids);
  }
  ...
  public Map<StopLocation, SortedMap<Layer, Region>> getRegionsByStops(
      Iterable<StopLocation> stops) {

    Map<Integer, StopLocation> stopsById = CollectionsLibrary.mapToValue(stops,
        "id", Integer.class);
    ...
    List<?> rows = _t2.findByNamedParam(query.toString(), "ids",
        stopsById.keySet());
    ...
    for (Object row : rows) {
      ...
      StopLocation stop = stopsById.get(id);
