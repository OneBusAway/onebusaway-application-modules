...
public class MetroKCDataLoader {

  public static void main(String[] args) throws Exception {
    ApplicationContext ctx = MetroKCApplicationContext.getApplicationContext();
    MetroKCDataLoader loader = new MetroKCDataLoader();
    ctx.getAutowireCapableBeanFactory().autowireBean(loader);
    loader.run();
  }
  ...
  @Autowired
  void setMetrokcDAO(MetroKCDAO dao) {
    _dao = dao;
  }
  ...
  private class CSVToEntityHandler implements CSVListener {
    ...
    private List<Object> _entities = new ArrayList<Object>(FLUSH_SIZE);
    ...
    public void handleLine(List<String> in) throws Exception {
      ...
      _entities.add(bean);
      if (_entities.size() >= FLUSH_SIZE)
        flush();
    }

    public void flush() throws StopException {
      _count += _entities.size();
      _dao.saveAllEntities(_entities);
      _entities.clear();
      if (_count % 1000 == 0)
        System.out.println("  flush: count=" + _count);
    }
  }
        ...
        if (IdentityBean.class.isAssignableFrom(type)) {
          ...
          try {
            ...
            if (value.length() > 0) {
              ...
              Object v = _dao.loadEntity(type, id);
              values.put(field, v);
            } else {
              toRemove.add(field);
            }
  ...
  private class ServicePatternKeyPropertyModifier implements PropertyModifier {
    ...
    public void modify(BeanWrapper wrapper, Map<String, Object> values)
      ...
      ChangeDate changeDate = _dao.loadEntity(ChangeDate.class, changeId);
  ...
  private class ServicePatternPropertyModifier implements PropertyModifier {
    ...
    public void modify(BeanWrapper wrapper, Map<String, Object> values)
      ...
      values.put("servicePattern", _dao.loadEntity(ServicePattern.class, id));
    }
  }
  ...
  private class TransLinkShapePointKeyPropertyModifier implements
    ...
    public void modify(BeanWrapper wrapper, Map<String, Object> values)
      ...
      TransLink link = _dao.loadEntity(TransLink.class, id);
  ...
  private class BlockTripKeyPropertyModifier implements PropertyModifier {
    ...
    public void modify(BeanWrapper wrapper, Map<String, Object> values)
      ...
      ChangeDate changeDate = _dao.loadEntity(ChangeDate.class, changeDateId);
      Trip trip = _dao.loadEntity(Trip.class, tripId);
