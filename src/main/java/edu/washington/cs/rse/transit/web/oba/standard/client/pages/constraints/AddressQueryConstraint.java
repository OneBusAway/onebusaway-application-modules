...
public class AddressQueryConstraint extends AbstractConstraint {
    ...
    public AddressQueryConstraint(String query) {
        ...
        LatLngBounds view = new LatLngBounds();
        view = view.extend(new LatLng(47.97430795395781, -121.79454591726969));
        view = view.extend(new LatLng(47.152554314370924, -122.50104172828858));
    ...
    private void handlePlacemarks(Placemark[] placemarks, final PlacemarkEvents events) {
        ...
        for (final Placemark mark : placemarks) {
            ...
            Image img = new Image(OneBusAwayStandardResources.INSTANCE.getImageMarker().getUrl());
    ...
    private class GeocoderResultHandler implements LocationCallback {
        ...
        public void onSuccess(Placemark[] placemarks) {
    ...
    private class MultiPlacemarkEvents extends PlacemarkEvents {
        ...
        @Override
        public void handlePlacemarkPre(Placemark mark) {
            _bounds = _bounds.extend(mark.getPoint());
        }

        @Override
        public void handlePlacemarkInfoWindowPost(Placemark mark, FlowPanel w, Marker marker) {
            LatLng p = mark.getPoint();
            String target = getTarget("index", KEY_MODE, MODE_LOCATION, "lat", p.getLatitude(), "lon",
                    p.getLongitude(), "accuracy", mark.getAccuracy());
            Hyperlink link = new Hyperlink(_msgs.standardIndexPageFindNearbyStops(), target);
            w.add(link);
        }
