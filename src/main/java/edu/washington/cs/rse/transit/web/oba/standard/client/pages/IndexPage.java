...
import edu.washington.cs.rse.transit.web.oba.common.client.widgets.SpanPanel;
import edu.washington.cs.rse.transit.web.oba.common.client.widgets.SpanWidget;
import edu.washington.cs.rse.transit.web.oba.standard.client.pages.constraints.AddressQueryConstraint;
import edu.washington.cs.rse.transit.web.oba.standard.client.pages.constraints.AreaConstraint;
import edu.washington.cs.rse.transit.web.oba.standard.client.pages.constraints.LocationConstraint;
import edu.washington.cs.rse.transit.web.oba.standard.client.pages.constraints.NoConstraint;
import edu.washington.cs.rse.transit.web.oba.standard.client.pages.constraints.RouteConstraint;
import edu.washington.cs.rse.transit.web.oba.standard.client.pages.constraints.StopConstraint;
import edu.washington.cs.rse.transit.web.oba.standard.client.pages.constraints.StopIdentificationConstraint;
import edu.washington.cs.rse.transit.web.oba.standard.client.pages.constraints.StopSelectionConstraint;
...
public class IndexPage extends AbstractPageSource implements
  ...
  private static LatLng _center = new LatLng(47.601533, -122.32933);
    ...
    if (MODE_LOCATION.equals(mode) || MODE_STOP.equals(mode)) {
      ...
      try {
        ...
        if (MODE_LOCATION.equals(mode))
          return new LocationConstraint(new LatLng(lat, lon), accuracy);
        else
          return new StopConstraint(new LatLng(lat, lon), accuracy);
      ...
      try {
        ...
        bounds = bounds.extend(new LatLng(lat1, lon1));
        bounds = bounds.extend(new LatLng(lat2, lon2));
      ...
      try {
        int route = Integer.parseInt(context.getParam(KEY_ROUTE));
  ...
  private void setStopSelection(Context context) {
    ...
    if (context.hasParam(STOP_KEY)) {
      try {
        _selectedStop = Integer.parseInt(context.getParam(STOP_KEY));
      } catch (NumberFormatException ex) {
  ...
  private class StopsHandler implements AsyncCallback<StopsBean> {
    ...
    public void onSuccess(StopsBean bean) {
      ...
      for (final StopBean stop : stops) {
        final LatLng p = new LatLng(stop.getLat(), stop.getLon());
    ...
    private Marker getCloseMarker(final StopBean stop, final LatLng p) {
      ...
      boolean isSelected = stop.getId() == _selectedStop;
  ...
  private class StopHandler implements AsyncCallback<StopWithRoutesBean> {
    ...
    public void onSuccess(StopWithRoutesBean bean) {
      ...
      String street = getCombinedIntersectionName(stop);
      ...
      String html = "<a href=\"stop.action?id=" + stop.getId() + "\">" + _msgs.standardIndexPageGetRealtimeArrivalInfo() + "</a>";
      ...
      for (final RouteBean route : bean.getRoutes()) {
        SpanWidget w = new SpanWidget(Integer.toString(route.getNumber()));
    ...
    private String getCombinedIntersectionName(StopBean stop) {
      String main = stop.getMainStreet();
      String cross = stop.getCrossStreet();

      if ("Unnamed".equals(cross)) {
        return main;
      } else if ("Unnamed".equals(main)) {
        return cross;
      }
