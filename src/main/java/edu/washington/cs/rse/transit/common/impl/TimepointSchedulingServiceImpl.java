...
@Component
public class TimepointSchedulingServiceImpl extends AbstractSchedulingService
    implements TimepointSchedulingService {
  ...
  public List<ScheduledArrivalTime> getPredictedArrivalsByTimepointId(
      int timepointId) throws NoSuchStopException {
    return getPredictedArrivalsByTimepointId(timepointId,
        System.currentTimeMillis());
  }

  public List<ScheduledArrivalTime> getPredictedArrivalsByTimepointId(
      int timepointId, long now) throws NoSuchStopException {

    checkMetroTimeOffset();

    TimingBean timing = getTiming(now);

    List<StopTime> interpolatedStops = _dao.getActiveStopTimesByTimepointAndTimeRange(
        timepointId, timing.getTimeOfWeekInMinutes(), MAX_WINDOW);
    return getPredictedArrivals(timepointId, interpolatedStops, timing);
  }

  public List<ScheduledArrivalTime> getPredictedArrivalsByTimepointIds(
      Set<Integer> ids) {
  ...
  public List<ScheduledArrivalTime> getPredictedArrivalsByTimepointIds(
      Set<Integer> ids, long time) {
    checkMetroTimeOffset();

    TimingBean timing = getTiming(time);

    Map<Timepoint, Set<StopTime>> interpolatedStops = _dao.getActiveStopTimesByTimepointsAndTimeRange(
        ids, timing.getTimeOfWeekInMinutes(), MAX_WINDOW);
    ...
    for (Map.Entry<Timepoint, Set<StopTime>> entry : interpolatedStops.entrySet()) {
      ...
      List<ScheduledArrivalTime> pats = getPredictedArrivals(timepoint.getId(),
          stopTimes, timing);
  ...
  public List<ScheduledArrivalTime> getPredictedArrivalsByServicePatterns(
      Set<ServicePatternKey> ids, TimingBean timing) {

    checkMetroTimeOffset();

    int minutes = timing.getTimeOfWeekInMinutes();

    List<StopTime> sts = _dao.getStopTimesByServicePatternsAndTimeRange(ids,
        minutes, MAX_WINDOW);
  ...
  private List<ScheduledArrivalTime> getPredictedArrivals(int timepointId,
      Collection<StopTime> stopTimes, TimingBean timing) {

    // Retreive the scheduled arrivals
    List<ScheduledArrivalTime> retro = getStopTimesAsScheduledArrivalTimes(
        stopTimes, timing);
    Map<RouteAndTimeKey, Set<ScheduledArrivalTime>> scheduledArrivals = getArrivalEstimatesByKey(retro);

    Map<RouteAndTimeKey, Set<BusArrivalEstimateBean>> estimates = getArrivalEstimatesByKey(
        timepointId, timing);
  ...
  private Map<RouteAndTimeKey, Set<ScheduledArrivalTime>> getArrivalEstimatesByKey(
    ...
    for (ScheduledArrivalTime sat : sats) {
      RouteAndTimeKey key = new RouteAndTimeKey(sat.getRoute().getNumber(),
          sat.getScheduledTime());
   ...
   * @return
   */
  private Map<RouteAndTimeKey, Set<BusArrivalEstimateBean>> getArrivalEstimatesByKey(
      int timepointId, TimingBean timing) {
    ...
    try {
      ...
      throw new IllegalStateException("where?", ex);
    } catch (Exception ex) {
      _log.log(Level.WARNING, "error retreiving predicted timepoint arrivals",
          ex);
    }
  ...
  /**
   * Organize the {@link BusArrivalEstimateBean} beans into sets based on their
   * route+scheduled arrival time {@link RouteAndTimeKey}.
   * 
   * @param beans
   * @param timing
   * @return
   ...
   * @return
   */
  private RouteAndTimeKey getArrivalEstimateAsKey(BusArrivalEstimateBean bean,
      TimingBean timing) {
  ...
  private void setPredictedArrivalTime(ScheduledArrivalTime sat,
      BusArrivalEstimateBean eat) {
