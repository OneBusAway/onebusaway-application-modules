...
public class StopTimeSearchOperationsTest {
  ...
  public void testGetStopTimeInstancesInRange() {
    ...
    LocalizedServiceId localizedServiceIdA = new LocalizedServiceId(serviceIdA, tz);
    LocalizedServiceId localizedServiceIdB = new LocalizedServiceId(serviceIdB, tz);
    
    StopTimeIndexImpl index = new StopTimeIndexImpl();

    for (StopTimeEntry stopTime : stopTimesA)
      index.addStopTime(stopTime, localizedServiceIdA);

    for (StopTimeEntry stopTime : stopTimesB)
      index.addStopTime(stopTime, localizedServiceIdB);

    Map<LocalizedServiceId, List<Date>> serviceIdsAndDates = new HashMap<LocalizedServiceId, List<Date>>();
    serviceIdsAndDates.put(localizedServiceIdA,
        Arrays.asList(yesterday, today, tomorrow));
    serviceIdsAndDates.put(localizedServiceIdB,
        Arrays.asList(yesterday, today, tomorrow));
