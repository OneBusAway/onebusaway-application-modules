...
public class RestApiLibrary {
  ...
  public RestApiLibrary(String protocol, String host, Integer port,
      String apiPrefix) {
    _host = host;
  ...
  public ArrayList<JsonObject> getJsonObjectsForString(String string)
     ...
     * if(!response.get("status").getAsString().equals("OK")) throw new
     * Exception("Response error: status was not OK"); } else throw new
     * Exception("Invalid response: no status element was found.");
     */
  ...
  /**
   * Logs the given message to the remote server. The message is sent as a post
   ...
   */
  public String log(String baseIdentifier, String component, Integer priority,
      String message) {
    ...
    try {
      ...
      log.error("Error opening http connection to url : {}",
          requestUrl.toString());
      e.printStackTrace();
    } finally {
      if (conn != null) {
  ...
  private HttpURLConnection getHttpURLConnection(URL requestUrl) {
    ...
    try {
      ...
      log.error("Error opening Http Connection for url : {}",
          requestUrl.toString());
      e.printStackTrace();
    }
