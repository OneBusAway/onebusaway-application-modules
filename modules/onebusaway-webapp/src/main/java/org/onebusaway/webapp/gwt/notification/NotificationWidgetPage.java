...
public class NotificationWidgetPage extends WhereCommonPage {
  ...
  public NotificationWidgetPage(ContextManager contextManager) {
    _methods = new ArrivalsAndDeparturesMethods();
    _methods.setMessages(GWT.<WhereMessages> create(WhereMessages.class));
  ...
  public Widget create(final Context context) throws PageException {
    ...
    DivPanel notificationOptionsPanel = new DivPanel("notificationOptions");
  ...
  private DivPanel getNotificationMinutesBeforePanel() {
    ...
    DivPanel minutesRow = new DivPanel("notificationMinutesBeforePanel");

    minutesRow.add(new SpanWidget("Notify me"));

    _minutesBeforeTextBox = new TextBox();
    _minutesBeforeTextBox.addStyleName("notificationMinutesBeforeTextBox");
  ...
  private DivPanel getMethodsPanel() {
    DivPanel methodsPanel = new DivPanel("notificationMethodsPanel");

    for (NotificationMethod method : _notificationMethods) {
      DivPanel row = new DivPanel("notificationMethodPanel");
  ...
  private DivPanel getSaveDefaultsPanel() {
    DivPanel panel = new DivPanel("notificationDefaultsPanel");
    panel.add(new SpanWidget("Make this the default for:",
        "notificationDefaultsText"));

    Button justThisStop = new Button("This Stop");
    justThisStop.addStyleName("notificationDefaultsButton");
    ...
    allStops.addStyleName("notificationDefaultsButton");
  ...
  private void refreshInterface(StopWithArrivalsAndDeparturesBean bean) {
    ...
    for (ArrivalAndDepartureBean dep : bean.getArrivalsAndDepartures()) {
      if (dep.getTripId().equals(_tripId)) {
  ...
  private void updateArrivalsAndDeparturePanel(ArrivalAndDepartureBean bean) {
    ...
    _arrivalsAndDeparturesTable.setText(1, 0, bean.getRouteShortName());
    ...
    String href = "trip.action?id=" + bean.getTripId() + "&stop="
        + bean.getStopId();
    destinationPanel.add(new Anchor(bean.getTripHeadsign(), href));
  ...
  private void refreshAlarm() {
    ...
    if (_minutesBefore < 0) {
      ...
      _notificationTimer.schedule(10 * 1000);
    }
  }
