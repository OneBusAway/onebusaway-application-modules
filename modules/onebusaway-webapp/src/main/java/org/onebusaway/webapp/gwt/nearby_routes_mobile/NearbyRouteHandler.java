...
import org.onebusaway.transit_data.model.NearbyRoutesBean;
...
class NearbyRouteHandler implements AsyncCallback<NearbyRoutesBean> {
  ...
  public void onSuccess(NearbyRoutesBean bean) {
    ...
    for (int row = 0; row < rows; row++) {
      ...
      for (int col = 0; col < columns; col++) {
        ...
        DivWidget widget = new DivWidget(route.getShortName());
  ...
  private class ClickHandler implements ClickListener {
    ...
    public ClickHandler(RouteBean route, List<StopBean> stops,
        DivPanel stopPanel, int column) {
      _route = route;
      _stops = stops;
  ...
  private class StopPanelHandler {
    ...
    public void activate(RouteBean route, List<StopBean> stops, DivPanel panel,
        int column) {
      ...
      for (StopBean stop : stops) {
        Anchor stopWidget = new Anchor("<div>" + stop.getName()
            + "</div><div class=\"arrivalsStopNumber\">Stop # " + stop.getId()
            + " - " + stop.getDirection() + " bound</div>", true,
            "stop.action?id=" + stop.getId());
