...
public abstract class ServiceIdCalendarDataOp implements Comparator<Date> {
  ...
  public abstract int getFromTime(ServiceIdCalendarServiceData data);

  public abstract int getToTime(ServiceIdCalendarServiceData data);
  ...
  public Date shiftTime(ServiceIdCalendarServiceData data, Date time) {
    long offset = getFromTime(data) * 1000;
    return new Date(time.getTime() - offset);
  }

  public int compareInterval(ServiceIdCalendarServiceData data, Date serviceDate, Date from, Date to) {
  ...
  private static class ArrivalsServiceDateTimeOp extends ServiceIdCalendarDataOp {
    ...
    @Override
    public int getFromTime(ServiceIdCalendarServiceData data) {
      return data.getLastArrival();
    }

    @Override
    public int getToTime(ServiceIdCalendarServiceData data) {
