...
public class TripPlannerGraphTaskImpl implements TripPlannerGraphTask {
  ...
  private Map<AgencyAndId, List<StopTimeEntryImpl>> getStopTimesByTripId(
    ...
    for (int x = 0; x < stopTimes.size(); x++) {
      ...
      AgencyAndId serviceId = stopTime.getTrip().getServiceId();
  ...
  private void interpolateArrivalAndDepartureTimes(List<StopTime> stopTimes,
    ...
    for (int i = 0; i < stopTimes.size(); i++) {
      ...
      boolean hasDeparture = stopTime.getDepartureTime() != -1;
      boolean hasArrival = stopTime.getArrivalTime() != -1;
  ...
  private void processTrips(DataLookup lookup, TripPlannerGraphImpl graph,
    ...
    for (Trip trip : trips) {
      ...
      RouteCollection routeCollection = _whereDao.getRouteCollectionsForRoute(trip.getRoute());
  ...
  private static class BlockTripComparator implements Comparator<Trip> {
    ...
    public int compare(Trip o1, Trip o2) {
      ...
      if (o1.getBlockSequenceId() != -1 && o2.getBlockSequenceId() != -1)
        return o1.getBlockSequenceId() - o2.getBlockSequenceId();

      int t1 = getFirstDepartureTime(o1);
      int t2 = getFirstDepartureTime(o2);

      return t1 - t2;
    }

    private int getFirstDepartureTime(Trip trip) {
      ...
      int minDepartureTime = Integer.MAX_VALUE;

      for (StopTime stopTime : stopTimes) {
        if (stopTime.getDepartureTime() != -1
            && stopTime.getDepartureTime() < minDepartureTime)
          minDepartureTime = stopTime.getDepartureTime();
      }

      if (minDepartureTime == Integer.MAX_VALUE)
        throw new IllegalStateException(
            "no StopTimes with departureTime != -1 for trip " + trip);
