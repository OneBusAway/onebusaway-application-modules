...
import org.onebusaway.transit_data_federation.impl.StopSequenceBlocksLibrary;
import org.onebusaway.transit_data_federation.impl.StopSequencesLibrary;
...
class RouteBeanServiceImpl implements RouteBeanService {
  ...
  private StopSequencesLibrary _stopSequencesLibrary;
  ...
  @Autowired
  public void setWherService(TransitDataFederationDao whereDao) {
    _whereDao = whereDao;
  }
  ...
  @Autowired
  public void setStopSequencesLibrary(StopSequencesLibrary library) {
    _stopSequencesLibrary = library;
  }

  @Cacheable
  public RouteBean getRouteForId(AgencyAndId id) {

    RouteCollection rc = _whereDao.getRouteCollectionForId(id);
  ...
  public StopsForRouteBean getStopsForRoute(AgencyAndId routeId) {
    ...
    RouteCollection routeCollection = _whereDao.getRouteCollectionForId(routeId);
  ...
  private List<EncodedPolylineBean> getEncodedPolylinesForRoute(RouteCollection routeCollection) {
  ...
  private StopsForRouteBean go(RouteCollection routeCollection) {
    ...
    List<StopSequence> sequences = _stopSequencesLibrary.getStopSequencesForTrips(stopTimesByTrip);

    StopSequenceBlocksLibrary lib = new StopSequenceBlocksLibrary();
    List<StopSequenceBlock> blocks = lib.getStopSequencesAsBlocks(sequences);
  ...
  private Set<AgencyAndId> getShapeIdsForStopSequenceBlock(StopSequenceBlock block) {
