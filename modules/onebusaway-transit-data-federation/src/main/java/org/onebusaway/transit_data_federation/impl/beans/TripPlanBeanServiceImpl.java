...
class TripPlanBeanServiceImpl implements TripPlanBeanService {
  ...
  @Autowired
  public void setTripNarrativeService(TripNarrativeService service) {
    _tripNarrativeService = service;
  }

  @Autowired
  public void setStopTimeNarrativeService(StopTimeNarrativeService service) {
    _stopTimeNarrativeService = service;
  }

  @Transactional
  public List<TripPlanBean> getTripsAsBeans(Collection<TripPlan> trips) {
    return getTripsAsBeans(trips);
  }
  ...
  private void handleVehicleDeparture(List<TripSegmentBean> segments,
    ...
    RouteBean routeBean = _routeBeanService.getRouteForId(tripEntry.getRouteId());
    TripBean tripBean = _tripBeanService.getTripForId(tripEntry.getId());

    String routeShortName = ApplicationBeanLibrary.getBestName(
        tripBean.getRouteShortName(), routeBean.getShortName());
  ...
  private void handleBlockTransfer(List<TripSegmentBean> segments,
    ...
    TripNarrative nextTripNarrative = _tripNarrativeService.getTripForId(nextTrip.getId());
